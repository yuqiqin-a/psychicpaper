...
只通过双链引用，减去了需要整理标签、整理文件夹等繁琐的维护工作。双链管理起来几乎不费精力，只需打上两个前括号就可以精确地通过搜索到自己笔记。需要调整笔记分类的时候只需要找到对应的MOC整理即可，极大减轻了只打扫、不产出的症状。
...
这个工作流已经非常可靠地运行了2年多了，至少已经调教到一个我可以非常舒服地记录的程度了，并且能够输出一个像这样的经验分享。当然，每个人使用Obsidian的方法非常不同。如果你还没有找到适合自己的那个方法，不妨每个都试试看，我的这套方法不失为一个起点。
